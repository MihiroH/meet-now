...
final class MeetingLinkExtractorTests: XCTestCase {
    ...
    func testExtractTeams() {
        ...
        let link = MeetingLinkExtractor.meetingLink(for: event)
        XCTAssertEqual(
            link?.absoluteString, "https://teams.microsoft.com/l/meetup-join/abc123percent20sign")
    }
    ...
    func testTrailingPunctuation() {
        ...
        XCTAssertEqual(
            MeetingLinkExtractor.meetingLink(for: event)?.absoluteString,
    ...
    func testUnsafeURLSchemes() {
        let event = EKEvent(eventStore: EKEventStore())

        // Pattern matches but scheme is unsafe
        event.notes = "Unsafe link: file://meet.google.com/abc-defg-hij"
        XCTAssertNil(MeetingLinkExtractor.meetingLink(for: event))

        event.notes = "Unsafe link: javascript://meet.google.com/abc-defg-hij"
        XCTAssertNil(MeetingLinkExtractor.meetingLink(for: event))

        event.notes = "Unsafe link: data://meet.google.com/abc-defg-hij"
        XCTAssertNil(MeetingLinkExtractor.meetingLink(for: event))

        // Pattern matches and scheme is safe
        event.notes = "Safe link: https://meet.google.com/abc-defg-hij"
        XCTAssertNotNil(MeetingLinkExtractor.meetingLink(for: event))

        event.notes = "Safe link: http://meet.google.com/abc-defg-hij"
        XCTAssertNotNil(MeetingLinkExtractor.meetingLink(for: event))
    }
}
